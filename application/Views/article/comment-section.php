...
<div id="comments-box">
	...
	<div class="comment row">
		<div class="col-2 col-md-1">
			<div class="user-img"
				style="background-image: url(<?=PROFILE_IMG_DIR."/".$comment->profile_img?>); border: 1px solid #ebebeb">
			</div>
		</div>
		<div class="col-md-8 col-10 mt-1">
			<h6>
				<a href="<?=URLROOT?>/profile?u=<?=$comment->username?>"
					class='text-decoration-none text-dark d-inline-block me-2'><?=ht($comment->display_name)?></a>
			...
			<button data-comment-id="<?=$comment->id?>"
				class="reaction-btn me-4 react-heart <?= $comment->is_liked ? "reacted" : "" ?>"
				title='<?=$comment->like_count?> user(s) liked this comment'>
				<img src="<?=URLROOT?>/assets/reaction-heart.png" alt="" class='reaction-img'
					style='pointer-events: none;'>
				<span class="ps-1 count-heart-<?=$comment->id?>"
					style='pointer-events: none;'><?=$comment->like_count?></span>
			</button>

			<small class="p-2 pt-0 d-inline-block mt-1 text-muted reply-btn" style='cursor: pointer' data-type='comment'
				data-mention='<?=$comment->username?>' data-parent-id='<?=$comment->id?>'
			...
			<div class="dropup m-0 p-0 d-inline-block float-end">
				...
				<ul class="dropdown-menu m-0 float-start comment-dropdown" aria-labelledby="dropdownMenuButton1">
					...
					<li class='delete-comment'><a class="dropdown-item" href="javascript:void(0)"
							data-comment-id="<?=$comment->id?>">Delete</a></li>
					<li class='edit-comment'><a class="dropdown-item" href="javascript:void(0)"
							data-comment-id="<?=$comment->id?>" data-type="comment"
							data-content="<?=ht($comment->content)?>">Edit</a></li>
					<?php endif; ?>
			...
			<button class="btn btn-outline-light text-primary mt-3 ms-0 border d-block expand-replies"
		...
		<div class="reply-box mt-4 row">
			...
			<div class="col-md-8 col-10 mt-1 row">
				<div class="col-3 col-md-2">
					<div class="user-img"
						style="background-image: url(<?=PROFILE_IMG_DIR."/".$reply->profile_img?>); border: 1px solid #ebebeb">
					</div>
				</div>
				<div class="col-md-10 col-9 mt-1">
					<h6>
						<a href="<?=URLROOT?>/profile?u=<?=$reply->username?>"
							class='text-decoration-none text-dark d-inline-block me-2'><?=ht($reply->display_name)?></a>
						<small class='text-muted fw-normal'
							id='username-<?=$reply->id?>'><?=date("d M Y (H:i)", strtotime($reply->created_at))?>
							<?= $comment->is_edited ? "(edited)" : "" ?></small>
					</h6>
					...
					<button data-comment-id="<?=$reply->id?>"
						class="reaction-btn me-4 react-heart <?= $reply->is_liked ? "reacted" : "" ?>"
						title='<?=$reply->like_count?> user(s) liked this reply'>
						<img src="<?=URLROOT?>/assets/reaction-heart.png" alt="" class='reaction-img'
							style='pointer-events: none;'>
						<span class="ps-1 count-heart-<?=$reply->id?>"
							style='pointer-events: none;'><?=$reply->like_count?></span>
					</button>

					<small class="p-2 pt-0 d-inline-block mt-1 text-muted reply-btn" data-type='reply'
						data-mention='<?=$reply->username?>' data-parent-id='<?=$comment->id?>'
						style='cursor: pointer; user-select: none;'>REPLY</small>
					...
					<div class="dropup m-0 p-0 d-inline-block float-end">
						...
						<ul class="dropdown-menu m-0 float-start comment-dropdown"
							aria-labelledby="dropdownMenuButton1">
							<?php if(($_SESSION['user_id'] ?? "") === $reply->user_id): ?>
							<li class='delete-comment'><a class="dropdown-item" href="javascript:void(0)"
									data-comment-id="<?=$reply->id?>">Delete</a></li>
							<li class='edit-comment'><a class="dropdown-item" href="javascript:void(0)"
									data-comment-id="<?=$reply->id?>" data-type="reply"
									data-content="<?=ht($reply->content)?>">Edit</a></li>
							<?php endif; ?>
							<li><a class="dropdown-item" href="javascript:void(0)"
									data-comment-id="<?=$reply->id?>">Report</a></li>
						</ul>
					</div>
				</div>
			</div>
	...
	<nav aria-label="..." class='d-inline-block mt-5 mb-0 pb-0'>
		<ul class="pagination">
			<li class="page-item <?=$data['current_page'] == 2 || $data['current_page'] == 1 ? "disabled" : "" ?>">
				<a class="page-link"
					href="<?=URLROOT?>/article/comments/<?="{$data['article']->article_id}/".($data['current_page'] - 1)?>"
					tabindex="-1" aria-disabled="true">Previous</a>
			</li>
			<?php for ($page = 2; $page <= $data['comment_page_count']; $page++): ?>
			<li class="page-item <?=$page === $data['current_page'] ? "active" : "" ?>">
				<a class="page-link"
					href="<?=URLROOT?>/article/comments/<?="{$data['article']->article_id}/".$page?>"><?=$page?></a>
			</li>
			<?php endfor; ?>
...
<div class="toast align-items-center text-white bg-dark border-0 center-toast m-auto fixed-bottom mb-4 text-center"
	role="alert" aria-live="assertive" aria-atomic="true" id='message-toast'
	style='width: 300px!important; padding: 2px; font-size: 15px;'>
	<div class="d-flex">
		<div class="toast-body container show-save text-center"></div>
