...
const uniq = document.querySelector("[name='uniq_id']").value;

const bodyLoader = new LiveLoader();
bodyLoader.addIdSelector("[name='last_following_id']");
bodyLoader.addBtn("#show-more");
bodyLoader.addParams({
    "profile_uniq": uniq
});
bodyLoader.addEndPoint("ajax/profile/load-following");

bodyLoader.addListener((response) => {    
  Object.entries(response.following).forEach(
      ([key, value]) => {
        const profile = value;
        let btn = '';
